...
abstract class InterpreterSuite[F[_]](interpreter: Interpreter[F])(implicit monad: MonadError[F, RuntimeError]) extends SimpleTestSuite {
  def assertSuccess[A](expr: Expr, expected: A, env: Env[F] = Env.create[F], limits: Limits = Limits.create)(implicit enc: ValueEncoder[F, A]): Unit =
    assertEquals(toEither(interpreter.eval(expr, env, limits)), Right(enc(expected)))

  def assertFailure(expr: Expr, expected: RuntimeError, env: Env[F] = Env.create[F], limits: Limits = Limits.create): Unit =
