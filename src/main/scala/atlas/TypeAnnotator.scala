...
object TypeAnnotator {
  ...
  def doBlock(block: BlockExpr): Step[TExpr] =
    pushScope {
      def preallocate(stmt: Expr): Step[Unit] =
        stmt match {
          case let: LetExpr =>
            for {
              tpe <- gen
              _   <- set(let.varName, tpe)
            } yield ()
      ...
      def doStmt(stmt: Expr): Step[TExpr] =
        stmt match {
          case let: LetExpr =>
            for {
              tpe    <- get(let.varName)
      ...
      for {
        tpe    <- gen
        _      <- block.stmts.traverse(preallocate)
        stmts  <- block.stmts.traverse(doStmt)
