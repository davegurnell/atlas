...
trait ValueDecoder[A] {
  def apply(value: Value): Either[Interpreter.Error, A]
}
...
trait ValueDecoderFunctions {
  ...
  def pure[A](func: Value => Either[Interpreter.Error, A]): ValueDecoder[A] =
    new ValueDecoder[A] {
      def apply(arg: Value): Either[Interpreter.Error, A] =
        func(arg)
    }
}
...
trait ValueDecoderInstances {
  ...
  implicit def listDecoder[A](implicit dec: ValueDecoder[A]): ValueDecoder[List[A]] =
    pure {
      ...
      case value       => Left(Interpreter.Error(s"Could not decode list: $value"))
    }
  ...
  implicit def circeDecoder[A](implicit dec: Decoder[A]): ValueDecoder[A] =
    pure { value =>
      def toJson(value: Value): Either[Interpreter.Error, Json] =
        value match {
          case Null        => Right(Json.Null)
          case True        => Right(Json.True)
          case False       => Right(Json.False)
          case Intr(num)   => Right(Json.fromInt(num))
          case Real(num)   => Json.fromDouble(num).toRight(Interpreter.Error(s"Could not decode double: $num"))
          ...
          case value: Func => Left(Interpreter.Error(s"Cannot decode function: $value"))
        }

      toJson(value).flatMap(_.as[A].leftMap(err => Interpreter.Error(err.message)))
    }
}
